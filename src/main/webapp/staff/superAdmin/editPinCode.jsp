		...
		<div class="col-md-4 offset-md-4">
			<div class="card paint-card">
				<div class="card-body">
					...
					<%
					int pincodeId = Integer.parseInt(request.getParameter("pincodeId"));
					PinCodeDao dao = new PinCodeDao(DbConnect.getCon());
					PinCode p = dao.getAllPinCodeById(pincodeId);
					%>

					<form action="../../editPinCode" method="post"
						onsubmit="return validate()">


						<div class="mb-3">
							<label class="form-label">Pin Code Name</label> <input
								name="pincode_name" type="text" class="form-control"
								 required aria-describedby="emailHelp"
								value="<%=p.getName()%>">
						</div>
						<input type="hidden" name="pincode_id" value="<%=p.getPincodeId()%>">
						
						<div class="mb-3">
								<label for="problem_type">Zone Name</label> 
								<select name="zone_name"
									name=problem_type class="form-select">							
									<option value="<%= p.getZoneId() %>"><%=p.getZoneName()%></option>
